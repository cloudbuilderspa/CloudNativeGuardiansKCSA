...
while True:
    idioma_input = input("Choose the language for the exam (inglés/español) or type (en/es): ").strip().lower()
    print()  # Añadir un salto de línea después de la selección del idioma

    if idioma_input in ["ingles", "inglés", "en"]:
        preguntas = preguntas_ingles
        idioma_seleccionado = "Inglés (English)"
        break
    elif idioma_input in ["espanol", "español", "es"]:
        preguntas = preguntas_espanol
        idioma_seleccionado = "Español (Spanish)"
        break
    else:
        print("Language not recognized. Please enter 'en' for English or 'es' for Spanish.")
        # Opcional: en lugar de volver a preguntar, se podría usar un idioma por defecto como antes:
        # print("Idioma no reconocido, se usará inglés por defecto. / Language not recognized, English will be used by default.")
        # preguntas = preguntas_ingles
        # idioma_seleccionado = "Inglés (English)"
        # break
